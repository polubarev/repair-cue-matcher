...
- **Worked example:** On 1,000 reminder calls we see:
  ...
  - If scheduling rates and average call durations are similar, we interpret Model A as having **better comprehension** (fewer clarifications needed). A move from 15 → 5 per 100 over a deployment cycle is a clear improvement; a jump from 5 → 15 would be a regression worth investigating (ASR config change, new prompt, etc.).
